...
Saves timestamped artifacts to models/ and updates models/latest symlink.
...
def smooth_target_encode(train_col, target, k=30):
    ...
    return train_col.map(lookup).fillna(global_mean), lookup
...
def main():
    ...
    _, ctr_pub_lookup   = smooth_target_encode(train["PUBLISHER_UUID"], train["CLICKED"])
    _, ctr_brand_lookup = smooth_target_encode(train["BRAND_ID"].astype(str), train["CLICKED"])
    ctr_lookups = {"pub": ctr_pub_lookup, "brand": ctr_brand_lookup}

    train_clicked = train[train["CLICKED"] == 1].copy()
    _, cvr_pub_lookup   = smooth_target_encode(train_clicked["PUBLISHER_UUID"], train_clicked["CONVERTED"])
    _, cvr_brand_lookup = smooth_target_encode(train_clicked["BRAND_ID"].astype(str), train_clicked["CONVERTED"])
    ...
    train_cl  = train_fe[train_fe["CLICKED"] == 1]
    val_cl    = val_fe[val_fe["CLICKED"] == 1]
    X_tr_cvr  = train_cl[FEATURES_CVR]; y_tr_cvr = train_cl["CONVERTED"]
    X_va_cvr  = val_cl[FEATURES_CVR];   y_va_cvr = val_cl["CONVERTED"]
    pos_w_cvr = int((y_tr_cvr == 0).sum() / (y_tr_cvr == 1).sum())

    print("\n--- Validation ---")
    print("Training CTR model (LightGBM)...")
    ctr_eval = LGBMClassifier(n_estimators=300, learning_rate=0.05, num_leaves=31,
                               is_unbalance=True, random_state=SEED, verbose=-1)
    ctr_eval.fit(X_tr, y_tr_ctr)
    ctr_metrics = evaluate(ctr_eval, X_va, y_va_ctr)
    print(f"  CTR  ROC-AUC={ctr_metrics['ROC-AUC']}  PR-AUC={ctr_metrics['PR-AUC']}  LogLoss={ctr_metrics['Log-loss']}")

    print("Training CVR model (XGBoost)...")
    cvr_eval = XGBClassifier(n_estimators=300, learning_rate=0.05, max_depth=4,
                              scale_pos_weight=pos_w_cvr, random_state=SEED, verbosity=0, eval_metric="logloss")
    cvr_eval.fit(X_tr_cvr, y_tr_cvr)
    cvr_metrics = evaluate(cvr_eval, X_va_cvr, y_va_cvr)
    print(f"  CVR  ROC-AUC={cvr_metrics['ROC-AUC']}  PR-AUC={cvr_metrics['PR-AUC']}  LogLoss={cvr_metrics['Log-loss']}")
    ...
    _, ctr_pub_full   = smooth_target_encode(events["PUBLISHER_UUID"], events["CLICKED"])
    _, ctr_brand_full = smooth_target_encode(events["BRAND_ID"].astype(str), events["CLICKED"])
    ctr_lookups_full  = {"pub": ctr_pub_full, "brand": ctr_brand_full}

    events_clicked = events[events["CLICKED"] == 1]
    _, cvr_pub_full   = smooth_target_encode(events_clicked["PUBLISHER_UUID"], events_clicked["CONVERTED"])
    _, cvr_brand_full = smooth_target_encode(events_clicked["BRAND_ID"].astype(str), events_clicked["CONVERTED"])
    cvr_lookups_full  = {"pub": cvr_pub_full, "brand": cvr_brand_full}
    ...
    pos_w_cvr_full = int((events_cl["CONVERTED"] == 0).sum() / (events_cl["CONVERTED"] == 1).sum())

    ctr_model = LGBMClassifier(n_estimators=300, learning_rate=0.05, num_leaves=31,
                                is_unbalance=True, random_state=SEED, verbose=-1)
    ctr_model.fit(events_fe[FEATURES], events_fe["CLICKED"])

    cvr_model = XGBClassifier(n_estimators=300, learning_rate=0.05, max_depth=4,
                               scale_pos_weight=pos_w_cvr_full, random_state=SEED, verbosity=0, eval_metric="logloss")
    ...
    artifact_path = MODELS_DIR / f"artifacts_{run_ts}.joblib"
    metrics_path  = MODELS_DIR / f"metrics_{run_ts}.json"

    joblib.dump(artifact, artifact_path)
    with open(metrics_path, "w") as f:
        json.dump({"run_ts": run_ts, "CTR": ctr_metrics, "CVR": cvr_metrics}, f, indent=2)

    # Update latest symlink
    latest = MODELS_DIR / "latest.joblib"
    if latest.is_symlink() or latest.exists():
        latest.unlink()
    latest.symlink_to(artifact_path.name)

    print(f"\nSaved  → {artifact_path}")
    print(f"Metrics→ {metrics_path}")
    print(f"Latest → {latest} → {artifact_path.name}")
